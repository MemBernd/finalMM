...
$decorations = (isset($_POST['decorations'])) ? $_POST['decorations'] : 'test description';
...
if ($result == -1) {
    ...
    $sql = " INSERT INTO `task` (`subject`, `description`, `priority`, `status`, `eventRecord`, `creator`, `assignee`)"
        . " VALUES ('Create tasks for event.', '".$description."', 'medium', 'created', ".$eventRecord.", 'janet', 'jack');";
    modify($sql);
    $sql = " INSERT INTO `task` (`subject`, `description`, `priority`, `status`, `eventRecord`, `creator`, `assignee`)"
        . " VALUES ('Create tasks for event.', '".$description."', 'medium', 'created', ".$eventRecord.", 'janet', 'natalie');";
    modify($sql);
